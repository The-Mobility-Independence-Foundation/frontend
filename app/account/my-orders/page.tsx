...
export default function AccountMyOrders() {
  ...
  const [orderAndStatusSelected, setOrderAndStatusSelected] = useState<{orderID: number | string, status: string}>();
  ...
  const getOrderStatusMenuItems = (orderStatus: string) => {
    const items: string[] = orderStatus.toLowerCase() == OrderStatus.PENDING ? 
    [
  ...
  const changeOrderStatus = async (orderID: number | string, status: string) => {
  ...
  const onOrderStatusDialogClose = async (confirmed: boolean) => {
    if(confirmed && orderAndStatusSelected) {
      ...
      await changeOrderStatus(orderAndStatusSelected.orderID, orderAndStatusSelected.status);
  ...
  return (
    <>
              ...
              {!loadingOrders &&
                orders.data.results.map((order) => (
                  <Order
                    ...
                    onMenuItemClick={(item) => onOrderMenuItemChange(item, order.id)}
                    className="mb-[1rem]"
                  />
                ))}
            </div>
      ...
      >
        <Dialog
          text={
            `Are you sure you\'d like to mark this order as ${orderAndStatusSelected?.status}`
          }
